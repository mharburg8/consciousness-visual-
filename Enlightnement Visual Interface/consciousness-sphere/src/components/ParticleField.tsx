...
function generateColors(count: number): Float32Array {
  const palette = [
    ...
    new THREE.Color('#f5e8c8'),
    new THREE.Color('#d4c8e8'),
    new THREE.Color('#c8d8e8'),
    new THREE.Color('#e8e4f0'),
  ];
...
// Smoothstep fade: 1.0 at dist ≤ near, 0.0 at dist ≥ far
...
export default function ParticleField() {
  ...
  const geo1 = useMemo(() => {
    ...
    g.setAttribute('position', new THREE.BufferAttribute(generatePositions(n), 3));
    g.setAttribute('color',    new THREE.BufferAttribute(generateColors(n),    3));
    return g;
  }, [count]);
  ...
  useFrame(({ clock, camera }) => {
    ...
    // Fully transparent when far out (dist > 10), fade in as camera approaches center
    const fade = distFade(dist, 3.5, 10);
    ...
    if (mat1Ref.current) mat1Ref.current.opacity = 0.55 * fade;
    if (mat2Ref.current) mat2Ref.current.opacity = 0.35 * fade;
  });
